 ...
 */
public class CriteriaNodeActionXMLImporterRuntime extends AbstractActionXMLImporterRuntime {
	/**
	...
	protected void importParams(final NodeActionValue action, final Element element) throws XMLImportException {
		final CriteriaElementXMLImporter importer = new CriteriaElementXMLImporter();
		final List<Element> elements = XML.getElementsByName(element, "element");
		if (elements.size() == 2) {
			final CriteriaElement configElement0 = importer.importFromElement(elements.get(0));
			final CriteriaElement configElement1 = importer.importFromElement(elements.get(1));
