 ...
 */
public class TransformingFormulaEnumeratorRuntime extends EnumeratorExtensionRuntime {
	/**
	...
	public List<String> listExtensions() throws JSException {
		List<Extension<TransformingFormulaExtensionRuntime<?>>> extensions = MandelbrotRegistry.getInstance().getTransformingFormulaRegistry().getExtensionList();
		List<String> references = new LinkedList<String>();
		for (Extension<TransformingFormulaExtensionRuntime<?>> extension : extensions) {
	...
	public JSExtension getExtension(final String extensionId) throws JSException {
		try {
			return new JSExtension(MandelbrotRegistry.getInstance().getTransformingFormulaExtension(extensionId));
		}
