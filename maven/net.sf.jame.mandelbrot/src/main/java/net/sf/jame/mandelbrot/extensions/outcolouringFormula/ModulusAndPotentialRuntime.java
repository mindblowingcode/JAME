 ...
 */
public class ModulusAndPotentialRuntime extends AbstractOutcolouringPaletteRuntime<ModulusAndPotentialConfig> {
	/**
	...
	protected int renderIndex(final RenderedPoint cp) {
		...
		return (int) Math.rint((0.5d + 0.5d * (modulus - Math.floor(modulus))) * (cp.time - 1));
	}
}
