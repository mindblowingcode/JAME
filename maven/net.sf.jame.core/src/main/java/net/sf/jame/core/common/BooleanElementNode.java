 ...
 */
public abstract class BooleanElementNode extends AttributeNode {
	public static final String NODE_CLASS = "node.class.BooleanElement";
	private final ConfigElementListener listener;
	private final ValueConfigElement<Boolean> configElement;

	/**
	 * @param nodeId
	 * @param configElement
	 */
	public BooleanElementNode(final String nodeId, final ValueConfigElement<Boolean> configElement) {
		super(nodeId);
		setNodeClass(BooleanElementNode.NODE_CLASS);
		this.configElement = configElement;
		listener = new ConfigElementListener();
		setNodeValue(new BooleanElementNodeValue(configElement.getValue()));
	}
	...
	protected NodeEditor createNodeEditor() {
		return new BooleanNodeEditor(this);
	}

	protected class BooleanNodeEditor extends NodeEditor {
		/**
		 * @param node
		 */
		public BooleanNodeEditor(final AttributeNode node) {
			super(node);
		}
		...
		protected void doSetValue(final NodeValue<?> value) {
			configElement.removeChangeListener(listener);
			configElement.setValue(((BooleanElementNodeValue) value).getValue());
			configElement.addChangeListener(listener);
		}
		...
		public Class<?> getNodeValueType() {
			return BooleanElementNodeValue.class;
		}
		...
		public NodeValue<?> createNodeValue(final Object value) {
			return new BooleanElementNodeValue((Boolean) value);
		}
	}
		...
		public void valueChanged(final ValueChangeEvent e) {
			...
			switch (e.getEventType()) {
				case ValueConfigElement.VALUE_CHANGED: {
					setNodeValue(new BooleanElementNodeValue((Boolean) e.getParams()[0]));
