 ...
 */
public abstract class AbstractConfigElementNodeActionXMLImporterRuntime<T extends ConfigElement> extends AbstractActionXMLImporterRuntime {
	/**
	...
	protected void importParams(final NodeActionValue action, final Element element) throws XMLImportException {
		...
		if (elements.size() == 2) {
			final ConfigElement configElement0 = importer.importFromElement(elements.get(0));
			final ConfigElement configElement1 = importer.importFromElement(elements.get(1));
			action.setActionParams(new Serializable[] { configElement0, configElement1 });
		}
	}
