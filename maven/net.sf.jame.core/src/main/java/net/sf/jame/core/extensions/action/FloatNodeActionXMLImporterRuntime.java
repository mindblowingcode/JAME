 ...
 */
public class FloatNodeActionXMLImporterRuntime extends AbstractActionXMLImporterRuntime {
	/**
	...
	protected void importParams(final NodeActionValue action, final Element element) throws XMLImportException {
		final FloatElementXMLImporter importer = new FloatElementXMLImporter();
		final List<Element> elements = XML.getElementsByName(element, "element");
		if (elements.size() == 2) {
			final FloatElement configElement0 = importer.importFromElement(elements.get(0));
			final FloatElement configElement1 = importer.importFromElement(elements.get(1));
