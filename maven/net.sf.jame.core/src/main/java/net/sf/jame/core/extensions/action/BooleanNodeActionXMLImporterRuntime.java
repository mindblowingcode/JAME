 ...
 */
public class BooleanNodeActionXMLImporterRuntime extends AbstractActionXMLImporterRuntime {
	/**
	...
	protected void importParams(final NodeActionValue action, final Element element) throws XMLImportException {
		final BooleanElementXMLImporter importer = new BooleanElementXMLImporter();
		final List<Element> elements = XML.getElementsByName(element, "element");
		if (elements.size() == 2) {
			final BooleanElement configElement0 = importer.importFromElement(elements.get(0));
			final BooleanElement configElement1 = importer.importFromElement(elements.get(1));
