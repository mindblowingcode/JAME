...
public class ListConfigElement<T extends ConfigElement> extends AbstractConfigElement {
	...
	public void appendElement(final T element) {
		...
		if (checkContext()) {
			...
			fireConfigChanged(new ValueChangeEvent(ListConfigElement.ELEMENT_ADDED, getContext().getTimestamp(), element, index));
		}
	}
	...
	public void insertElementAfter(final int index, final T element) {
		...
		if (checkContext()) {
			element.setContext(getContext());
			fireConfigChanged(new ValueChangeEvent(ListConfigElement.ELEMENT_INSERTED_AFTER, getContext().getTimestamp(), element, index));
		}
	}
	...
	public void insertElementBefore(final int index, final T element) {
		...
		if (checkContext()) {
			element.setContext(getContext());
			fireConfigChanged(new ValueChangeEvent(ListConfigElement.ELEMENT_INSERTED_BEFORE, getContext().getTimestamp(), element, index));
		}
	}
	...
	public void removeElement(final int index) {
		...
		if (checkContext()) {
			fireConfigChanged(new ValueChangeEvent(ListConfigElement.ELEMENT_REMOVED, getContext().getTimestamp(), element, index));
		}
	}
	...
	public void moveElementUp(final int index) {
		...
		if (index > 0) {
			...
			if (checkContext()) {
				element.setContext(getContext());
				fireConfigChanged(new ValueChangeEvent(ListConfigElement.ELEMENT_MOVED_UP, getContext().getTimestamp(), element, index));
			}
		}
	}
	...
	public void moveElementDown(final int index) {
		...
		if (index < elements.size() - 1) {
			...
			if (checkContext()) {
				element.setContext(getContext());
				fireConfigChanged(new ValueChangeEvent(ListConfigElement.ELEMENT_MOVED_DOWN, getContext().getTimestamp(), element, index));
			}
		}
	}
	...
	public void setElement(final int index, final T element) {
		...
		if (checkContext()) {
			element.setContext(getContext());
			fireConfigChanged(new ValueChangeEvent(ListConfigElement.ELEMENT_CHANGED, getContext().getTimestamp(), element, index));
		}
	}
