...
public class ComplexNodeActionXMLImporterRuntime extends AbstractActionXMLImporterRuntime {
	...
	protected void importParams(final NodeActionValue action, final Element element) throws XMLImportException {
		final ComplexElementXMLImporter importer = new ComplexElementXMLImporter();
		final List<Element> elements = XML.getElementsByName(element, "element");
		if (elements.size() == 2) {
			final ComplexElement configElement0 = importer.importFromElement(elements.get(0));
			final ComplexElement configElement1 = importer.importFromElement(elements.get(1));
