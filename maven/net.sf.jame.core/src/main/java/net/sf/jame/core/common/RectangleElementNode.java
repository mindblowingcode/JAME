 ...
 */
public class RectangleElementNode extends AttributeNode {
	public static final String NODE_CLASS = "node.class.RectangleElement";
	private final ConfigElementListener listener;
	private final ValueConfigElement<Rectangle> configElement;

	/**
	 * @param nodeId
	 */
	public RectangleElementNode(final String nodeId, final ValueConfigElement<Rectangle> configElement) {
		super(nodeId);
		setNodeClass(RectangleElementNode.NODE_CLASS);
		this.configElement = configElement;
		listener = new ConfigElementListener();
		setNodeValue(new RectangleElementNodeValue(configElement.getValue()));
	}
	...
	protected NodeEditor createNodeEditor() {
		return new RectangleNodeEditor(this);
	}

	protected class RectangleNodeEditor extends NodeEditor {
		/**
		 * @param node
		 */
		public RectangleNodeEditor(final AttributeNode node) {
			super(node);
		}
		...
		protected void doSetValue(final NodeValue<?> value) {
			configElement.removeChangeListener(listener);
			configElement.setValue(((RectangleElementNodeValue) value).getValue());
			configElement.addChangeListener(listener);
		}
		...
		public Class<?> getNodeValueType() {
			return RectangleElementNodeValue.class;
		}
		...
		public NodeValue<?> createNodeValue(final Object value) {
			return new RectangleElementNodeValue((Rectangle) value);
		}
	}
		...
		public void valueChanged(final ValueChangeEvent e) {
			...
			switch (e.getEventType()) {
				case ValueConfigElement.VALUE_CHANGED: {
					setNodeValue(new RectangleElementNodeValue((Rectangle) e.getParams()[0]));
