 ...
 */
public class ViewNodeActionXMLImporterRuntime extends AbstractActionXMLImporterRuntime {
	/**
	...
	protected void importParams(final NodeActionValue action, final Element element) throws XMLImportException {
		final ViewElementXMLImporter importer = new ViewElementXMLImporter();
		final List<Element> elements = XML.getElementsByName(element, "element");
		if (elements.size() == 2) {
			final ViewElement configElement0 = importer.importFromElement(elements.get(0));
			final ViewElement configElement1 = importer.importFromElement(elements.get(1));
