...
import net.sf.jame.service.spool.job.DistributedJobEncoder;
...
public abstract class AbstractSpoolJobService implements JobService<SpoolJobInterface> {
	...
	private class ExecutorTask implements Runnable {
		...
		public void run() {
			if (session == null) {
				logger.error("Failed to send message " + message + ", session is null");
				return;
			}
			try {
				...
				if (!session.isExpired()) {
					...
					logger.error("Failed to send message " + message + " on session " + session.getSessionId());
					session.invalidate();
				}
			}
			catch (final Exception e) {
	...
	private class JobSession implements SessionHandler {
		...
		public synchronized void update() {
			...
			if (oldStatus != status) {
				if (logger.isDebugEnabled()) {
					logger.debug("Job " + job.getJobId() + " status changed from " + oldStatus.getName() + " to " + status.getName());
				}
				oldStatus = status;
				synchronized (messageMonitor) {
					messageDirty = true;
