...
import net.sf.jame.contextfree.common.FillRuleElementNodeValue;
 ...
 */
public class FillRuleElementEditorRuntime extends EditorExtensionRuntime {
	/**
	...
	private class EditorComponent extends JPanel implements NodeEditorComponent {
		...
		public EditorComponent(final NodeEditor nodeEditor) {
			...
			ruleComboBox = GUIFactory.createComboBox(new FillRuleComboBoxModel(), ContextFreeSwingExtensionResources.getInstance().getString("tooltip.fillRule"));
			ruleComboBox.setRenderer(new FillRuleListCellRenderer());
			ruleComboBox.setSelectedItem((((FillRuleElementNodeValue) nodeEditor.getNodeValue()).getValue()));
	...
	private class NodeEditorActionListener implements ActionListener {
		...
		public void actionPerformed(final ActionEvent e) {
			...
			if (!rule.equals(nodeEditor.getNodeValue().getValue())) {
				nodeEditor.setNodeValue(new FillRuleElementNodeValue(rule));
			}
		}
	}

	private class FillRuleComboBoxModel extends DefaultComboBoxModel {
		private static final long serialVersionUID = 1L;

		public FillRuleComboBoxModel() {
			addElement("even-odd");
			addElement("not-zero");
		}
	}

	private class FillRuleListCellRenderer extends DefaultListCellRenderer {
		private static final long serialVersionUID = 1L;
