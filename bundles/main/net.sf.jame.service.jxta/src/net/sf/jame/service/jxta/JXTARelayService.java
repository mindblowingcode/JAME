 ...
 */
public class JXTARelayService {
	private static final Logger logger = Logger.getLogger(JXTARelayService.class);
	 ...
	 * @param processor
	 */
	public JXTARelayService(final File workdir) {
	...
	public void start() {
		if (thread == null) {
			...
			thread.setName("RelayService Thread");
			running = true;
			thread.start();
		}
	}
	...
	private class NetworkHandler implements Runnable {
		...
		public void run() {
			...
			try {
				...
				manager = new NetworkManager(NetworkManager.ConfigMode.RELAY, "JXTARelay", cache.toURI());
