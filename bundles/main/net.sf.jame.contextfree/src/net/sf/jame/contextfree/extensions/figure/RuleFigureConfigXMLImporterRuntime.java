...
public class RuleFigureConfigXMLImporterRuntime extends ExtensionConfigXMLImporterExtensionRuntime {
	...
	private class RuleFigureConfigXMLImporter extends XMLImporter<RuleFigureConfig> {
		...
		protected void importProperties(final RuleFigureConfig extensionConfig, final List<Element> propertyElements) throws ExtensionException, XMLImportException {
			...
			importPropability(extensionConfig, propertyElements.get(1));
		...
		private void importPropability(final RuleFigureConfig extensionConfig, final Element element) throws XMLImportException {
			final List<Element> propabilityElements = this.getElements(element, FloatElement.CLASS_ID);
			if (propabilityElements.size() == 1) {
				extensionConfig.setPropability(new FloatElementXMLImporter().importFromElement(propabilityElements.get(0)).getValue());
			}
		}
