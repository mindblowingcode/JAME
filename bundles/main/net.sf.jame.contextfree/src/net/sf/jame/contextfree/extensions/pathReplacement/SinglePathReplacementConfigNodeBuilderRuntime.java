 ...
 */
public class SinglePathReplacementConfigNodeBuilderRuntime extends NodeBuilderExtensionRuntime {
	/**
	...
	public NodeBuilder createNodeBuilder(final ExtensionConfig config) {
		return new ConfigNodeBuilder((SinglePathReplacementConfig) config);
	}

	private class ConfigNodeBuilder extends AbstractExtensionConfigNodeBuilder<SinglePathReplacementConfig> {
		/**
		 * @param config
		 */
		public ConfigNodeBuilder(final SinglePathReplacementConfig config) {
			super(config);
		}
		...
		public void createNodes(final Node parentNode) {
			parentNode.appendChildNode(new PathElementNode(getConfig()));
			parentNode.appendChildNode(new PathAdjustmentListElementNode(getConfig()));
		}

		private class PathElementNode extends StringElementNode {
			/**
			 * @param config
			 */
			public PathElementNode(final SinglePathReplacementConfig config) {
				super(config.getExtensionId() + ".path", config.getPathElement());
				setNodeLabel(ContextFreeResources.getInstance().getString("node.label.PathElement"));
			}
		}
		private class PathAdjustmentListElementNode extends AbstractConfigElementListNode<PathAdjustmentConfigElement> {
			 ...
			 * @param config
			 */
			public PathAdjustmentListElementNode(final SinglePathReplacementConfig config) {
