...
import net.sf.jame.twister.extensions.frame.layer.image.TextConfig;
import net.sf.jame.twister.swing.adapter.DefaultInputAdapterRuntime;

/**
 * @author Andrea Medeghini
 */
public class TextInputAdapterRuntime extends DefaultInputAdapterRuntime {
	/**
	 * 
	 */
	public TextInputAdapterRuntime() {
	}
	...
	public void processMouseDragged(final MouseEvent e) {
		final TextConfig config = (TextConfig) getAdapterContext().getConfig();
		if (config != null) {
			final double w = getRenderContext().getImageSize().getX();
			final double h = getRenderContext().getImageSize().getY();
			final DoubleVector2D center = new DoubleVector2D((e.getX() * 100d) / w, (e.getY() * 100d) / h);
			config.getContext().updateTimestamp();
			config.setLeft(center.getX());
			config.setTop(center.getY());
		}
	}
