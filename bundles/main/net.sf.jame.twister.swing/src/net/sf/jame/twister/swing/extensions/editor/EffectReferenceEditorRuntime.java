 ...
 */
public class EffectReferenceEditorRuntime extends ConfigurableReferenceEditorRuntime {
	/**
	...
	protected ConfigurableExtensionComboBoxModel createModel() {
		return new ConfigurableExtensionComboBoxModel(TwisterRegistry.getInstance().getEffectRegistry(), true);
	}
	...
	protected NodeValue createNodeValue(final ConfigurableExtensionReference reference) {
		// return new EffectExtensionReferenceNodeValue(reference != null ? reference.clone() : null);
		return new EffectExtensionReferenceNodeValue(reference);
	}
}
