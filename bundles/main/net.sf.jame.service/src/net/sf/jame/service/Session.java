...
public class Session {
	...
	public synchronized void open() throws SessionException {
		try {
			if (Session.logger.isTraceEnabled()) {
				Session.logger.trace("Open connection");
			}
	...
	public synchronized void close() throws SessionException {
		try {
			if (connection != null) {
				if (Session.logger.isTraceEnabled()) {
					Session.logger.trace("Close connection");
				}
				connection.close();
				connection = null;
				savepoint = null;
	...
	public synchronized void openTransaction() throws TransactionException, InterruptedException {
		while (savepoint != null) {
			if (Session.logger.isTraceEnabled()) {
				Session.logger.trace("Waiting for transaction...");
			}
			wait(10000);
		}
		try {
			transactionId += 1;
			if (Session.logger.isTraceEnabled()) {
				Session.logger.trace("Open transaction: " + transactionId);
			}
	...
	public synchronized void abortTransaction() throws TransactionException {
		try {
			if (Session.logger.isTraceEnabled()) {
				Session.logger.trace("Abort transaction: " + transactionId);
			}
			if (savepoint == null) {
	...
	public synchronized void closeTransaction() throws TransactionException {
		try {
			if (Session.logger.isTraceEnabled()) {
				Session.logger.trace("Close transaction: " + transactionId);
			}
			if (savepoint == null) {
				throw new TransactionException("Transaction not opened: " + transactionId);
			}
