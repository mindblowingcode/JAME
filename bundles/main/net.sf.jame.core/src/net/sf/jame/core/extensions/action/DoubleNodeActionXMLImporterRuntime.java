 ...
 */
public class DoubleNodeActionXMLImporterRuntime extends AbstractActionXMLImporterRuntime {
	/**
	...
	protected void importParams(final NodeActionValue action, final Element element) throws XMLImportException {
		final DoubleElementXMLImporter importer = new DoubleElementXMLImporter();
		final List<Element> elements = XML.getElementsByName(element, "element");
		if (elements.size() == 2) {
			final DoubleElement configElement0 = importer.importFromElement(elements.get(0));
			final DoubleElement configElement1 = importer.importFromElement(elements.get(1));
