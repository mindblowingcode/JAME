 ...
 */
public class StringNodeActionXMLImporterRuntime extends AbstractActionXMLImporterRuntime {
	/**
	...
	protected void importParams(final NodeActionValue action, final Element element) throws XMLImportException {
		final StringElementXMLImporter importer = new StringElementXMLImporter();
		final List<Element> elements = XML.getElementsByName(element, "element");
		if (elements.size() == 2) {
			final StringElement configElement0 = importer.importFromElement(elements.get(0));
			final StringElement configElement1 = importer.importFromElement(elements.get(1));
