 ...
 */
public abstract class ShortElementNode extends AttributeNode {
	public static final String NODE_CLASS = "node.class.ShortElement";
	private final ConfigElementListener listener;
	private final ValueConfigElement<Short> configElement;

	/**
	 * @param nodeId
	 * @param configElement
	 */
	public ShortElementNode(final String nodeId, final ValueConfigElement<Short> configElement) {
		super(nodeId);
		setNodeClass(ShortElementNode.NODE_CLASS);
		this.configElement = configElement;
		listener = new ConfigElementListener();
		setNodeValue(new ShortElementNodeValue(configElement.getValue()));
	}
	...
	protected NodeEditor createNodeEditor() {
		return new ShortNodeEditor(this);
	}

	protected class ShortNodeEditor extends NodeEditor {
		/**
		 * @param node
		 */
		public ShortNodeEditor(final AttributeNode node) {
			super(node);
		}
		...
		protected void doSetValue(final NodeValue<?> value) {
			configElement.removeChangeListener(listener);
			configElement.setValue(((ShortElementNodeValue) value).getValue());
			configElement.addChangeListener(listener);
		}
		...
		public Class<?> getNodeValueType() {
			return ShortElementNodeValue.class;
		}
		...
		public NodeValue<?> createNodeValue(final Object value) {
			return new ShortElementNodeValue((Short) value);
		}
	}
		...
		public void valueChanged(final ValueChangeEvent e) {
			...
			switch (e.getEventType()) {
				case ValueConfigElement.VALUE_CHANGED: {
					setNodeValue(new ShortElementNodeValue((Short) e.getParams()[0]));
