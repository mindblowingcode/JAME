 ...
 */
public class FontNodeActionXMLImporterRuntime extends AbstractActionXMLImporterRuntime {
	/**
	...
	protected void importParams(final NodeActionValue action, final Element element) throws XMLImportException {
		final FontElementXMLImporter importer = new FontElementXMLImporter();
		final List<Element> elements = XML.getElementsByName(element, "element");
		if (elements.size() == 2) {
			final FontElement configElement0 = importer.importFromElement(elements.get(0));
			final FontElement configElement1 = importer.importFromElement(elements.get(1));
