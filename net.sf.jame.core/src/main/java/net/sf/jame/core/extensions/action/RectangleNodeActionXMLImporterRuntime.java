 ...
 */
public class RectangleNodeActionXMLImporterRuntime extends AbstractActionXMLImporterRuntime {
	/**
	...
	protected void importParams(final NodeActionValue action, final Element element) throws XMLImportException {
		final RectangleElementXMLImporter importer = new RectangleElementXMLImporter();
		final List<Element> elements = XML.getElementsByName(element, "element");
		if (elements.size() == 2) {
			final RectangleElement configElement0 = importer.importFromElement(elements.get(0));
			final RectangleElement configElement1 = importer.importFromElement(elements.get(1));
