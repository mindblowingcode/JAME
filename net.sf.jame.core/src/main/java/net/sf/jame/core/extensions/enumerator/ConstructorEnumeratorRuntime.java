...
public class ConstructorEnumeratorRuntime extends EnumeratorExtensionRuntime {
	...
	public List<String> listExtensions() throws JSException {
		List<Extension<ConstructorExtensionRuntime>> extensions = CoreRegistry.getInstance().getConstructorRegistry().getExtensionList();
		List<String> references = new LinkedList<String>();
		for (Extension<ConstructorExtensionRuntime> extension : extensions) {
	...
	public JSExtension getExtension(final String extensionId) throws JSException {
		try {
			return new JSExtension(CoreRegistry.getInstance().getConstructorExtension(extensionId));
		}
