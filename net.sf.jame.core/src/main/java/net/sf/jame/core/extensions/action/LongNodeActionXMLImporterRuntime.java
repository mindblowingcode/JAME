 ...
 */
public class LongNodeActionXMLImporterRuntime extends AbstractActionXMLImporterRuntime {
	/**
	...
	protected void importParams(final NodeActionValue action, final Element element) throws XMLImportException {
		final LongElementXMLImporter importer = new LongElementXMLImporter();
		final List<Element> elements = XML.getElementsByName(element, "element");
		if (elements.size() == 2) {
			final LongElement configElement0 = importer.importFromElement(elements.get(0));
			final LongElement configElement1 = importer.importFromElement(elements.get(1));
