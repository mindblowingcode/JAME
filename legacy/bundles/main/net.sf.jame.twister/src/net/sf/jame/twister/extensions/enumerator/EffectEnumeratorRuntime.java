 ...
 */
public class EffectEnumeratorRuntime extends EnumeratorExtensionRuntime {
	/**
	...
	public List<String> listExtensions() throws JSException {
		List<Extension<EffectExtensionRuntime<?>>> extensions = TwisterRegistry.getInstance().getEffectRegistry().getExtensionList();
		List<String> references = new LinkedList<String>();
		for (Extension<EffectExtensionRuntime<?>> extension : extensions) {
	...
	public JSExtension getExtension(final String extensionId) throws JSException {
		try {
			return new JSExtension(TwisterRegistry.getInstance().getEffectExtension(extensionId));
		}
