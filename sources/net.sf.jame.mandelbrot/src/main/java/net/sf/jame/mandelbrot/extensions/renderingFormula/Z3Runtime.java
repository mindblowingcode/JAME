 ...
 */
public class Z3Runtime extends AbstractRenderingFormulaRuntime<Z3Config> {
	...
	public int renderPoint(final RenderedPoint cp) {
		...
		for (int k = 1; k <= iterations; k++) {
			ta = cp.zr * cp.zr;
			tb = cp.zi * cp.zi;
			cp.zr = (ta * cp.zr) - (3d * tb * cp.zr) + cp.wr;
			cp.zi = (3d * ta * cp.zi) - (tb * cp.zi) + cp.wi;
			if (formulaRuntime != null) {
	...
	public double getNormalizedIterationCount(final RenderedPoint cp) {
		...
		// return cp.time + 1d - Math.log(Math.log(modulus)) / Math.log(3d);
		return cp.time + (Math.log(Math.log(threshold)) - Math.log(Math.log(modulus))) / Math.log(3d);
	}
}
