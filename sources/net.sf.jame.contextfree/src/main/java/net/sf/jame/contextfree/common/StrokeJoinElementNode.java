 ...
 */
public abstract class StrokeJoinElementNode extends AttributeNode {
	public static final String NODE_CLASS = "node.class.StrokeJoinElement";
	 ...
	 * @param configElement
	 */
	public StrokeJoinElementNode(final String nodeId, final ValueConfigElement<String> configElement) {
		super(nodeId);
		setNodeClass(StrokeJoinElementNode.NODE_CLASS);
		this.configElement = configElement;
		listener = new ConfigElementListener();
		setNodeValue(new StrokeJoinElementNodeValue(configElement.getValue()));
		setNodeLabel(ContextFreeResources.getInstance().getString("node.label.StrokeJoinElement"));
	}
	...
	protected NodeEditor createNodeEditor() {
		return new StrokeJoinNodeEditor(this);
	}

	protected class StrokeJoinNodeEditor extends NodeEditor {
		/**
		 * @param node
		 */
		public StrokeJoinNodeEditor(final AttributeNode node) {
			super(node);
		}
		...
		protected void doSetValue(final NodeValue<?> value) {
			configElement.removeChangeListener(listener);
			configElement.setValue(((StrokeJoinElementNodeValue) value).getValue());
			configElement.addChangeListener(listener);
		}
		...
		public Class<?> getNodeValueType() {
			return StrokeJoinElementNodeValue.class;
		}
		...
		public NodeValue<?> createNodeValue(final Object value) {
			return new StrokeJoinElementNodeValue((String) value);
		}
	}
		...
		public void valueChanged(final ValueChangeEvent e) {
			...
			switch (e.getEventType()) {
				case ValueConfigElement.VALUE_CHANGED: {
					setNodeValue(new StrokeJoinElementNodeValue((String) e.getParams()[0]));
