...
public class ImageEnumeratorRuntime extends EnumeratorExtensionRuntime {
	...
	public List<String> listExtensions() throws JSException {
		List<Extension<ImageExtensionRuntime<?>>> extensions = TwisterRegistry.getInstance().getImageRegistry().getExtensionList();
		List<String> references = new LinkedList<String>();
		for (Extension<ImageExtensionRuntime<?>> extension : extensions) {
	...
	public JSExtension getExtension(final String extensionId) throws JSException {
		try {
			return new JSExtension(TwisterRegistry.getInstance().getImageExtension(extensionId));
		}
