 ...
 */
public abstract class AbstractColorRendererConfigXMLImporter<T extends ColorRendererExtensionConfig> extends XMLImporter<T> {
	/**
	 * @return
	 */
	 ...
	 */
	protected abstract void importProperties(Element element, T config, List<Element> propertyElements) throws XMLImportException;
	...
	public T importFromElement(final Element element) throws XMLImportException {
		...
		if (propertyElements.size() == this.getPropertiesSize()) {
			this.importProperties(element, config, propertyElements);
		}
		return config;
	}
}
