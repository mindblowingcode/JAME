 ...
 */
public class RenderingFormulaEnumeratorRuntime extends EnumeratorExtensionRuntime {
	/**
	...
	public List<String> listExtensions() throws JSException {
		List<Extension<RenderingFormulaExtensionRuntime<?>>> extensions = MandelbrotRegistry.getInstance().getRenderingFormulaRegistry().getExtensionList();
		List<String> references = new LinkedList<String>();
		for (Extension<RenderingFormulaExtensionRuntime<?>> extension : extensions) {
	...
	public JSExtension getExtension(final String extensionId) throws JSException {
		try {
			return new JSExtension(MandelbrotRegistry.getInstance().getRenderingFormulaExtension(extensionId));
		}
