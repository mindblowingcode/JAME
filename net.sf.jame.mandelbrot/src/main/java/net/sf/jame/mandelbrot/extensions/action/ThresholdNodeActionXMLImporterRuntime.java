...
public class ThresholdNodeActionXMLImporterRuntime extends AbstractActionXMLImporterRuntime {
	...
	protected void importParams(final NodeActionValue action, final Element element) throws XMLImportException {
		final ThresholdElementXMLImporter importer = new ThresholdElementXMLImporter();
		final List<Element> elements = XML.getElementsByName(element, "element");
		if (elements.size() == 2) {
			final ThresholdElement configElement0 = importer.importFromElement(elements.get(0));
			final ThresholdElement configElement1 = importer.importFromElement(elements.get(1));
