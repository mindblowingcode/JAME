 ...
 */
public class OrbitTrapEnumeratorRuntime extends EnumeratorExtensionRuntime {
	/**
	...
	public List<String> listExtensions() throws JSException {
		List<Extension<OrbitTrapExtensionRuntime<?>>> extensions = MandelbrotRegistry.getInstance().getOrbitTrapRegistry().getExtensionList();
		List<String> references = new LinkedList<String>();
		for (Extension<OrbitTrapExtensionRuntime<?>> extension : extensions) {
	...
	public JSExtension getExtension(final String extensionId) throws JSException {
		try {
			return new JSExtension(MandelbrotRegistry.getInstance().getOrbitTrapExtension(extensionId));
		}
