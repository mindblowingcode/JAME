...
import net.sf.jame.core.common.ShortElementNodeValue;
 ...
 */
public class ShortElementEditorRuntime extends EditorExtensionRuntime {
	/**
	...
	private class EditorComponent extends JPanel implements NodeEditorComponent {
		...
		private class FieldListener implements ActionListener, FocusListener {
			...
			public void actionPerformed(final ActionEvent e) {
				Short value = ((ShortElementNodeValue) nodeEditor.getNodeValue()).getValue();
				try {
				...
				if (!nodeEditor.getNodeValue().getValue().equals(value)) {
					nodeEditor.setNodeValue(new ShortElementNodeValue(value));
				}
			}
